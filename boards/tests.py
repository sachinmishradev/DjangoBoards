...
class Home(TestCase):
    def testForUrlStatus(self):
        url = reverse('home')
        response = self.client.get(url)
        self.assertEqual(response.status_code,200)

    def testForViews(self):
        view = resolve('/')
...
class BoardTopicsTest(TestCase):
    ...
    def test_for_link_back_to_the_homepage(self):
        b_url = reverse('board_topics',kwargs={'pk':1})
        response = self.client.get(b_url)
